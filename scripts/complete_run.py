...
def check_log_files(log_path, output_file):
    ...
    with open(output_file, "w") as f:
        ...
        for file in Path(log_path).rglob("*.log"):
            with open(file) as r:
                ...
                if "ERROR" in text:
                    for line in text.splitlines():
                        if "ERROR" in line:
                            if n_errors ==0:
                                f.write(f"{output[0]} succesfully generated at {datetime.utcnow().strftime("%d/%m/%y %H:%M")} with errors \n")
            ...
            text=None
        if n_errors ==0:
            f.write(f"{output[0]} succesfully generated at {datetime.utcnow().strftime("%d/%m/%y %H:%M")} with no errors \n")
...
argparser.add_argument("--fileDBconfig", help="fileDBconfig", type=dict, required=True)
argparser.add_argument("--filelist", help="filelist", type=str, required=True)
argparser.add_argument("--gen_output", help="gen_output", type=str)
argparser.add_argument("--summary_log", help="summary_log", type=str)
argparser.add_argument("--fileDB", help="fileDB", type=str)
args = argparser.parse_args()

check_log_files(args.log_path, args.summary_log)
