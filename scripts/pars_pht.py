...
channel_dict = configs.on(args.timestamp, system=args.datatype)['snakemake_rules']['pars_pht']["inputs"]['par_pht_config'][args.channel]
...
# run aoe cal
if aoe_options.pop("run_aoe") ==True:

    if "pdf" in aoe_options:
        pdf =eval(aoe_options.pop("pdf"))
    else:
        pdf = standard_aoe
    ...
    cal_dict, out_dict, plot_dict, aoe_obj = aoe_calibration(final_dict, lh5_path=f'{args.channel}/dsp', cal_dicts=cal_dict, 
                            ...
                            mean_func=mean_func, sigma_func=sigma_func, **aoe_options) 
...
for out in sorted(args.hit_pars):
    ...
    final_hit_dict = {"pars":{ "operations":cal_dict[fk.timestamp]} , 
                    "results": {"ecal":results_dicts[fk.timestamp], "partition_ecal": ecal_results,"aoe": out_dict}}
...
for out in args.aoe_results:
    fk = ChannelProcKey.get_filekey_from_pattern(os.path.basename(out))
    final_object_dict = {"ecal": object_dict[fk.timestamp],
